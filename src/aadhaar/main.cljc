(ns aadhaar.main
  "Kotodama WASM entrypoint for the Aadhaar clean-room actor (L4) — Clojure port.
  ...
  facts (`aadhaar.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Identity"            :plural "identities"            :id-prefix "aadhaar_ide"
   ...
   {:entity "Document"            :plural "documents"             :id-prefix "aadhaar_doc"
    ...
    :coerce {}                                   :refs {:identityId "Identity"}}
   {:entity "VerificationRequest" :plural "verificationrequests"   :id-prefix "aadhaar_ver"
...
(defn emit-facts
  "EAVT facts for one record: {\"aadhaar.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "aadhaar-compat" :tier tier :entities entities} 200])
...
(defn start! [] :aadhaar-compat/ready)
