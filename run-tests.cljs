(ns run-tests
  ...
  test namespaces:
    ...
    aadhaar.main-test  (.cljc) -- portable, listed below
    aadhaar.kotoba-qualification-test  (.clj) -- JVM-only, NOT listed

  The qualification namespace requires kotoba.compiler.core, kotoba.runtime
  and kotoba.wasm-exec to compile src/aadhaar/page_limit.kotoba and execute it on
  ...
  Measured 2026-08-20 -- the two runtimes agree on the shared namespace:
    ...
    clojure -M:test   ->  Ran 9 tests containing 93 assertions
    this runner       ->      7 tests containing 89 assertions

  The difference is exactly 2 deftest(s) and 4 assertions, which is the
